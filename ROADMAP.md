...
- ~~Share cards + dynamic Open Graph images~~ — ✅ **shipped**: every "Share my calls" / "Share the run" now produces a canonical `/s?…` link whose OG image (`/api/og`, edge-rendered champion card: crown, logo, outlasted field, streak) unfurls on X/WhatsApp/LinkedIn; the landing page funnels into the game. Remaining from the original idea: per-result URLs for individual matchups (currently day-level + run-level).
