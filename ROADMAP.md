...
- **The Arena500 — a fixed-size, reconstituted index** — the head-to-head arena holds a fixed ~500 companies at a time (the most notable growth/late startups), chosen as the top 500 by a notability score. S&P-500-style reconstitution: companies are *promoted in* as they rise (raise a big round, hire up, gain momentum) and *relegated out* as they fade or graduate (IPO / acquire). This is the core scaling mechanism — it keeps votes concentrated so the Elo stays credible no matter how big the underlying database gets — and it's a legible flagship ("the Arena500 index of private startups") whose promotions/relegations are recurring, shareable content. The full open DB (tens of thousands, all stages) lives underneath and is surfaced through Discover; the Arena500 is the curated ranking on top.
